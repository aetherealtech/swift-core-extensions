...
//    }
    ...
    func testNext() async throws {
        try await withTimeout(timeInterval: 1.0) {
            ...
            await withCheckedContinuation { continuation in
                ...
                Task {
                    value.value = await publisher.buffer(size: .max, prefetch: .keepFull, whenFull: .dropOldest).values.next()
                }
            }
    ...
    @MainActor
    func testNextCompletes() async throws {
        try await withTimeout(timeInterval: 1.0) {
            let publisher = PassthroughSubject<Int, Never>()
    ...
    func testWaitForNext() async throws {
        try await withTimeout(timeInterval: 1.0) {
            ...
            nonisolated(unsafe) let completed = CurrentValueSubject<Bool?, Never>(nil)
            ...
            await withCheckedContinuation { continuation in
                ...
                Task {
                    completed.value = await publisher.buffer(size: .max, prefetch: .keepFull, whenFull: .dropOldest).values.waitForNext()
                }
            }
                        
            try assertNil(completed.value)
            
            let expectedValue = Int.random(in: 0..<100)
            
            publisher.send(expectedValue)
            
            for await completed in completed.values {
    ...
    func testWaitUntil() async throws {
        try await withTimeout(timeInterval: 1.0) {
            ...
            await withCheckedContinuation { continuation in
                ...
                Task {
                    value.value = await publisher.buffer(size: .max, prefetch: .keepFull, whenFull: .dropOldest).values.waitUntil { $0 >= 50 }
                }
            }
    ...
    func testWaitUntilAsync() async throws {
        try await withTimeout(timeInterval: 1.0) {
            ...
            await withCheckedContinuation { continuation in
                ...
                Task {
                    value.value = await publisher.buffer(size: .max, prefetch: .keepFull, whenFull: .dropOldest).values.waitUntil { value async in value >= 50 }
                }
            }
    ...
    func testWaitUntilNotNil() async throws {
        try await withTimeout(timeInterval: 1.0) {
            ...
            await withCheckedContinuation { continuation in
                ...
                Task {
                    value.value = await publisher.buffer(size: .max, prefetch: .keepFull, whenFull: .dropOldest).values.waitUntilNotNil()
                }
            }
    ...
    func testWaitUntilNilTrue() async throws {
        try await withTimeout(timeInterval: 1.0) {
            ...
            await withCheckedContinuation { continuation in
                ...
                Task {
                    value.value = await publisher.buffer(size: .max, prefetch: .keepFull, whenFull: .dropOldest).values.waitUntilNil()
                }
            }
    ...
    func testWaitFor() async throws {
        try await withTimeout(timeInterval: 1.0) {
            ...
            await withCheckedContinuation { continuation in
                ...
                Task {
                    result.value = await publisher.buffer(size: .max, prefetch: .keepFull, whenFull: .dropOldest).values.wait(for: value)
                }
            }
